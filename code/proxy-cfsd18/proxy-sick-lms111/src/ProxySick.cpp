...
odcore::data::dmcp::ModuleExitCodeMessage::ModuleExitCode ProxySick::body()
{
  ...
  while (getModuleStateAndWaitForRemainingTimeInTimeslice() == odcore::data::dmcp::ModuleStateMessage::RUNNING) {
    ...
    if (counter == 10) {
      /*
      fstream m_file;
      m_file.open("lidarData.txt",ios::out | ios::app);
      m_file << "Hello World!";
      m_file.close();*/
...
{
  const unsigned char settingsModeString[] = {0x02, 0x73, 0x4D, 0x4E, 0x20, 0x6D, 0x4C, 0x4D, 0x50, 0x73, 0x65, 0x74, 0x73, 0x63, 0x61, 0x6E, 0x63, 0x66, 0x67, 0x20, 0x31, 0x33, 0x38, 0x38, 0x20, 0x31, 0x20, 0x30, 0x39, 0x43, 0x34, 0x20, 0x46, 0x46, 0x46, 0x39, 0x32, 0x32, 0x33, 0x30, 0x20, 0x32, 0x32, 0x35, 0x35, 0x31, 0x30, 0x03};
  const string settingString(reinterpret_cast<char const *>(settingsModeString), 48);
//  std::shared_ptr<UDPSender> udpsender(UDPFactory::createUDPSender(m_udpReceiverIP, m_udpPort));
...
{
  const unsigned char settingsModeString[] = {0x02, 0x73, 0x57, 0x4E, 0x20, 0x4C, 0x4D, 0x50, 0x6F, 0x75, 0x74, 0x70, 0x75, 0x74, 0x52, 0x61, 0x6E, 0x67, 0x65, 0x20, 0x31, 0x20, 0x30, 0x39, 0x43, 0x34, 0x20, 0x46, 0x46, 0x46, 0x39, 0x32, 0x32, 0x33, 0x30, 0x20, 0x32, 0x32, 0x35, 0x35, 0x31, 0x30, 0x03};
  const string settingString(reinterpret_cast<char const *>(settingsModeString), 43);
//  std::shared_ptr<UDPSender> udpsender(UDPFactory::createUDPSender(m_udpReceiverIP, m_udpPort));
  m_sick->send(settingString);
}
